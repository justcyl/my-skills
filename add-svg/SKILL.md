...
当科研 draw.io 图需要补充可识别的生物结构、器官、仪器、动物或其他复杂对象时触发本 skill。先选择素材来源（网络候选或自行绘制），记录来源与许可，再将单一 SVG 图元插入并完成尺寸、布局、连接线和导出检查。

本 skill 不下载后执行未知远程代码，不把未经许可的素材直接当作最终资产，也不重复绘制已经插入的同一对象。若简单 draw.io 原生图元已经足够，应优先保持编辑性并避免无意义装饰。
