...
当用户提出“科研流程图”“论文配图”“技术路线图”“机制示意图”“图形摘要”或“draw.io 作图”等需求时触发本 skill。默认产出可编辑的 `.drawio` 源文件，并按需生成 SVG/PDF/PNG 预览；如果用户只给出主题而未给出完整拓扑，先明确科学信息、图的角色和主要阅读方向，再开始布局。

本 skill 只负责科研图的结构、绘制、审查和导出，不替用户臆造实验结果、统计数值或因果结论。涉及公式、生物结构、模型模块或外部 SVG 时，遵循对应参考文件和导出 QA 规则；任何外部素材的来源与许可应在工作记录中保留。
