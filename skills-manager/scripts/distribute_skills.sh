...
usage() {
  cat >&2 <<'TEXT'
usage:
  bash scripts/distribute_skills.sh sync [--agent <codex|claude-code|pi|all>] [--skill-id <id>] [--mode <auto|symlink|copy>] [--dry-run]
  bash scripts/distribute_skills.sh status [--agent <codex|claude-code|pi|all>] [--skill-id <id>]
...
command_sync() {
  ...
  case "${target_agent}" in
    all) agents=(codex claude-code pi) ;;
    codex|claude-code|pi) agents=("${target_agent}") ;;
...
command_archive() {
  ...
  for agent in codex claude-code pi; do
    target_base="$(resolve_agent_target_dir "${agent}")"
