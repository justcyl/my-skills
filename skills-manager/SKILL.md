...
2. 用户要创建新 skill、重建 skill 骨架、评测 skill 或优化 description：
   读 `references/scene-create-skill.md`
   该模块只负责创建场景路由与状态入口。
   具体创建、评测（测试集、benchmark、viewer 审阅、盲评）、description 优化交给 `creator/SKILL.md`。
...
### 2. Create And Evaluate Skills

当用户要新建 skill、评测 skill 或优化 description：
...
3. 把具体内容生产与评测工作路由给 `creator/SKILL.md`。
   creator 提供完整的评测循环：设计测试集、运行 with-skill/baseline 对比、grading、benchmark 聚合、viewer 审阅、盲评与迭代修订。
