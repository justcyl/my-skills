---
name: skills-manager
description: 统一处理所有 skills 操作的总控 skill，包括搜索、导入、创建、编辑、优化、上游跟踪、分发、归档与发布。默认将所有下载/托管技能集中到  `~/project/my-skills` 仓库管理；即使在其他路径触发，也应回到该仓库执行管理流程。
---
...
1. 开始管理任何 skill 前，先检查仓库是否干净：
   - 一律在 `my-skills` 仓库执行：`git -C /Users/chenyl/project/my-skills status --short`
   - 若非空，必须由 agent 直接执行 git 命令提交现有改动（不得通过脚本代提）后再继续
   - 提交流程：`git -C /Users/chenyl/project/my-skills add -A && git -C /Users/chenyl/project/my-skills commit -m "<message>"`
2. 任何导致 `my-skills` 仓库变化的操作，都要在结束前完成：
   ...
   - 分发到 agent
   - 提交并推送仓库
3. 结束前再次检查 `git -C /Users/chenyl/project/my-skills status --short`：
   - 若非空，必须由 agent 直接执行 git 命令提交改动（不得通过脚本代提）
   - 目标状态是工作区干净后再结束本次管理
4. 对外部 skill：
...
2. 用户要创建新 skill、把经验沉淀成 skill、重建 skill 骨架、评测 skill、评估改进 skill 或优化 description：
...
- `bash scripts/sync_skill_state.sh ...`
...
6. 运行 `bash scripts/sync_skill_state.sh --skill-id <id> --push`。
...
2. 判断目标 skill 是否已存在：检查 `my-skills/<skill-id>/SKILL.md` 是否存在。
   如果不存在，先用 `bash scripts/create_skill.sh ...` 创建骨架与 `.skills/workspaces/<id>/`。
...
4. 子技能完成后，回到 manager 执行 `bash scripts/sync_skill_state.sh --skill-id <id> --push`。
...
2. 手动编辑后的状态刷新与审计，使用 `bash scripts/sync_skill_state.sh ...`。
...
1. 定位问题文件，只修复本次暴露的问题。
2. 运行 `bash scripts/sync_skill_state.sh --skill-id skills-manager --push`。
