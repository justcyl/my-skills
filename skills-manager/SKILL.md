---
name: skills-manager
description: 统一处理所有 skills 操作的总控 skill，包括搜索、导入、创建、重构、编辑、优化、上游跟踪、分发、归档与发布。当涉及到 SKILL 的编辑修改新建时，请用 skill-manager 来管理！
---
...
1. **编辑 / 同步 / 分发 / 归档已有 skill**：读 `references/manage.md`
2. **找 skill / 导入外部 skill / 更新上游**：读 `references/import.md`
3. **新建 skill / 从对话沉淀 skill / 重写 skill**：读 `references/create.md`
4. **评测 / 迭代改进已有 skill**（仅用户明确要求时）：读 `references/eval.md`
5. **优化 skill 的 description 触发效果**（仅用户明确要求时）：读 `references/description.md`
