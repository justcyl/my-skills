...
1. 开始管理任何 skill 前，先检查仓库是否干净：
   - 一律在 `my-skills` 仓库执行：`git -C ~/project/my-skills status --short`
   - 若工作区不干净，请先使用 git 命令提交现有改动后再继续
2. 任何导致 `my-skills` 仓库变化的操作，都要在结束前完成：
   - 状态同步
   - 规则审计
   - 必要的大模型审阅
   - 分发到 agent
   - 提交并推送仓库
3. 结束前再次检查 `git -C ~/project/my-skills status --short`：
   - 若工作区不干净，请执行 git 命令提交改动
   - 目标状态是工作区干净后再结束本次管理
4. 对外部 skill：
   - 搜索统一使用 `bash skills-manager/scripts/find_or_import_skill.sh search <query>`
...
1. 用户要找 skill、下载 skill、导入外部 skill、更新外部 skill、查看上游信息：
   读 `references/scene-find-and-import.md`
   该模块已吸收原 `find-skills` 的触发话术、搜索呈现与导入决策流程。
2. 用户要创建新 skill、把经验沉淀成 skill、重构 skill、评测 skill、评估改进 skill 或优化 description：
   读 `references/scene-create-skill.md`
   该模块只负责创建场景路由与状态入口。
   具体工作交给 `creator/SKILL.md`，creator 内部按任务类型继续路由：
   - 新建 / 重写 skill → creator 读 `creator/references/create-flow.md`
   - 评测 / 评估改进 / 迭代修订（跑测试集、benchmark、grading、盲评、viewer 审阅、根据反馈改进 skill）→ creator 读 `creator/references/eval-flow.md`
   - 优化 description 触发效果 → creator 读 `creator/references/description-flow.md`
...
1. 先理解需求与目标场景。
2. 如果来源不明确，先用 `bash skills-manager/scripts/find_or_import_skill.sh search <query>` 搜索。
3. 选定来源后，用 `bash skills-manager/scripts/find_or_import_skill.sh import ...` 导入到仓库根目录。
4. 脚本完成后，人工查看规则审计结果；必要时做语义审计。
5. 直接在根目录 skill 上完成中文优化与说明书整理。
6. 运行 `bash skills-manager/scripts/sync_skill_state.sh --skill-id <id> --push`。

### 2. Create, Evaluate And Improve Skills

当用户要新建 skill、评测 skill、评估改进 skill 或优化 description：

1. 先用 `references/scene-create-skill.md` 判断是"新建 / 重写 / 评测改进 / description 优化"。
2. 判断目标 skill 是否已存在：检查 `my-skills/<skill-id>/SKILL.md` 是否存在。
   如果不存在，先用 `bash skills-manager/scripts/create_skill.sh ...` 创建骨架与 `.skills/workspaces/<id>/`。
3. 把具体工作路由给 `creator/SKILL.md`，creator 内部按任务继续路由：
   - 新建 / 重写 → `creator/references/create-flow.md`
   - 评测 / 评估改进 / 迭代修订 → `creator/references/eval-flow.md`（含测试集设计、with-skill/baseline 对比、grading、benchmark 聚合、viewer 审阅、盲评、根据反馈修订 skill）
   - 优化 description → `creator/references/description-flow.md`
4. 子技能完成后，回到 manager 执行 `bash skills-manager/scripts/sync_skill_state.sh --skill-id <id> --push`。
...
2. 手动编辑后的状态刷新与审计，使用 `bash skills-manager/scripts/sync_skill_state.sh ...`。
3. 分发、状态查询、归档使用 `bash skills-manager/scripts/distribute_skills.sh ...`。
4. 默认分发源是仓库根目录 skill。
...
1. 定位问题文件，只修复本次暴露的问题。
2. 运行 `bash skills-manager/scripts/sync_skill_state.sh --skill-id skills-manager --push`。
