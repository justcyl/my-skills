...
description: 使用 paper-helper (ph) CLI 进行学术论文检索、导入、BibTeX 写入与渐进式阅读。当 agent 需要搜索论文、调研研究方向、导入指定论文、获取完整 metadata、添加到 .bib 文件、或精读全文时使用。覆盖完整工作流：需求澄清 → 查询构造 → 快路径（search/add）→ 慢路径（fetch）→ SQL 查询本地库。适用场景包括"帮我找关于 X 的论文"、"调研 Y 方向的工作"、"导入指定论文"、"获取完整 metadata"、"添加到 BibTeX"、"精读全文"、"搜索 Z 作者的研究"、"查一下本地收录了哪些论文"。
---

# ph-paper-helper

使用 `ph`（paper-helper）CLI 进行学术论文检索、导入、BibTeX 写入与渐进式阅读。
...
```bash
# 添加单篇论文到 .bib（自动 import + 补全 + 写入）
ph add --input arxiv://1706.03762 --bib refs.bib

# 批量添加
ph add --input arxiv://1706.03762 --input doi://10.1145/3025453 --bib refs.bib

# 跳过元数据补全（写入不完整的 entry，带警告）
ph add --input arxiv://1706.03762 --bib refs.bib --skip-metadata

# 预检不写文件
ph add --input arxiv://1706.03762 --bib refs.bib --dry-run
```

### 内部流程

1. `parse_uri` → `lookup_by_identifier`（跨 scheme 查 DB）→ 命中则跳过 API
2. 未命中 → 调 S2 API 获取基础信息 → `upsert_paper` 入库
3. 补全 bib 元数据（S2 + Crossref），除非 `--skip-metadata`
4. `paper_to_bibtex` 渲染 BibTeX 条目
5. 读取现有 .bib → 按 `% paper_id: xxx` 注释去重 → 已有则更新，新增则追加
6. `atomic_write_text` 原子写 .bib
...
手写的条目（没有 `% paper_id:` 注释）会被原样保留，不受影响。

### 输出字段

| 字段 | 说明 |
|------|------|
| `paper_id` | 规范化 paper_id |
| `add_state` | `added` / `updated` / `exists` / `dry-run` |
| `bib_path` | .bib 文件路径 |
| `metadata_state` | `completed` / `incomplete` / `already_ready` / `skipped` |
| `bib_ready` | bib 元数据是否完整 |
