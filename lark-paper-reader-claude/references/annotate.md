# Annotated Reader Layer For Claude Code
...
- 执行本 skill 时必须默认使用 Claude Code 的 Agent 工具派生 subagent 生成候选，不把"是否启用多智能体"作为需要用户确认的步骤；相互独立的 subagent 在同一条消息里并行发起。
- 多智能体候选覆盖：逐段翻译与覆盖检查、术语边注、公式直觉、图表读法、作者思考路径、引用背景、代码映射、视觉质量检查。
- subagent 的 prompt 必须自包含：工作目录绝对路径、输入文件、输出文件绝对路径、格式约定；subagent 看不到主对话上下文。
- 若当前环境确无 Agent 工具可用，或权限策略阻止调用，由主 agent 分批处理，并在 `annotations.json` 与 `qc-report.md` 写明未启用子代理的具体原因。
- subagent 候选必须由主 agent 复核后再写入飞书；不要直接把候选原样灌入文档。写飞书的 `lark-cli` 操作统一由主 agent 执行。
