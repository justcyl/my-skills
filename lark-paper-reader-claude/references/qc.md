# Quality Check (QC) Gate For Lark Paper Reader Claude

QC 指 Quality Check / 质量检查。交付前必须做结构检查；有问题就修复并重跑。Claude Code 具备图片查看能力（Read 工具可直接读 PNG），视觉检查默认必须执行，不得以"无视觉能力"为由跳过。
...
11. 正式文档不得包含执行过程、工具限制、权限判断、代理使用状态或 checkpoint/QC 说明；这些信息只能存在于内部文件和最终交付说明中。
...
视觉检查：用 Read 工具逐页（长文档可抽样，但必须覆盖公式密集页、图表页和 callout 密集页）查看 PNG；页数多时可派生 subagent 分片检查，每个 subagent 的 prompt 写明 PNG 路径列表和检查项，返回问题清单。
