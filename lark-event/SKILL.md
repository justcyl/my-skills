...
name: lark-event
version: 1.0.0
description: "飞书/Lark 实时事件监听、订阅和消费：通过 `lark-cli event consume <EventKey>` 输出 NDJSON 事件流，覆盖 IM 消息/表情/群变更、视频会议结束、妙记生成、画板更新等。适用于机器人实时消息处理、长时间订阅、webhook/push handler；支持 `--max-events`、`--timeout` 有界运行和 stderr ready-marker，方便 AI Agent 作为子进程编排。"
metadata:
  requires:
    bins: ["lark-cli"]
