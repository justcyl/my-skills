...
1. **固定舞台 + 等比缩放**：`#stage` 固定 1280×720，所有排版用绝对 px；
   ...
   固定坐标系是排版永不破版的前提。
2. **设计 token + 语义组件**：颜色只在 `:root` 定义，纪律是"颜色即
   角色"——每个语义色全场固定代表一个叙事角色，语义色不超过 3 个；
   每种修辞成分一个 class（金句、对比卡片、流程图、大数字、
...
1. **大纲先行**：每张 slide 写一句论断式标题（不是"结果"而是"X 提升了
   ...
   内容→组件决策表为每页标注版式。自检信号（非硬性）：纯文字版式
   连排多页时停下来问一遍——这些内容的形态是不是其实是对比/趋势/
   流程，能画就画。大纲连同版式标注一起与用户确认后再动手。
...
3. **定设计 token**：优先从 `references/design-catalog.md` 的主题预设
   ...
   `:root` 变量。颜色即角色：`--accent`/`--foil`/`--teal` 只是默认
   角色位（主角/对照/中性），按主题实际分配——无对抗性的主题
   `--foil` 可整场缺席；角色多于 3 个时不加第四色，改用分组或同色
   深浅（见 design-catalog §4）。不要引入灰色文字——层级靠字号和
   字重区分。
4. **逐张生成 slide**：每张一个 `<section class="slide">`，只用语义组件
   拼装。缺组件时先看 `references/anatomy.md` 的组件清单；确实没有就
   按 `references/design-catalog.md` §4 的生成语法造新组件——符合语法
   的新组件天然和全场和谐（样式进 `<style>` 统一管理，不写内联）。
