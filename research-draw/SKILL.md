...
当用户提出“科研插图”“论文配图”“图形摘要”“技术路线图”或希望把参考图转成可编辑 draw.io 文件时触发本 skill。它负责先建立科学信息与构图参考，再调用 `research-drawio-skill` 完成可编辑重建；需要复杂生物、医学或仪器对象时，调用 `add-svg` 选择或制作 SVG 图元。

本 skill 不把栅格参考图冒充为最终可编辑源，也不臆造实验数据、统计结果或因果关系。最终交付前应完成导出预览、文字完整性检查和规定的视觉对比循环。
