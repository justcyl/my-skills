---
name: external-repo
description: 研究和探索外部 GitHub 仓库。当需要了解开源项目的架构、API、代码实现或文档时使用。支持 DeepWiki、gh CLI、浅克隆三种策略，模型自主选择。
---

# External Repo — 外部仓库研究

研究任意公开 GitHub 仓库的架构、API、代码实现和文档。三种策略可灵活组合。
...
| 策略 | 适用场景 | 工具 |
|------|---------|------|
| DeepWiki | 热门仓库、架构/API/概念问题 | MCP 工具 或 WebFetch |
| gh CLI | 特定文件查看、README、元数据、未覆盖仓库 | `gh api` + `jq` |
| 本地浅克隆 | 小仓库全面搜索、跨文件 grep、前两者不足时 | `extract_repo.py` |
...
| 信号 | 推荐策略 |
|------|---------|
| 热门仓库（stars > 1k）+ 架构/API 问题 | DeepWiki |
| 需要快速了解仓库概况或文档结构 | DeepWiki |
| 非热门仓库 / DeepWiki 无覆盖 | gh CLI |
| 需要查看特定文件内容或最近提交 | gh CLI |
| 小仓库 + 需要跨多个文件搜索代码模式 | 本地浅克隆 |
| 需要 grep / ast-grep / fd 搜索 | 本地浅克隆 |
| 前两种策略信息不足，需要更深入分析 | 本地浅克隆 |

常见组合：先用 DeepWiki 获取架构概览，再用 gh CLI 查看具体文件。

## 策略 1：DeepWiki

适用于被 DeepWiki 收录的公开仓库（约 5 万+ 热门项目）。

### MCP 工具（已配置，直接调用）

| 工具 | 用途 |
|------|------|
| `mcp__deepwiki__read_wiki_structure` | 获取仓库文档目录结构 |
| `mcp__deepwiki__read_wiki_contents` | 获取具体文档页内容 |
| `mcp__deepwiki__ask_question` | 向 AI 提问关于仓库的问题 |

### WebFetch 方式

将 `github.com` 替换为 `deepwiki.com` 即可：

```
WebFetch https://deepwiki.com/vercel/next.js "路由系统是怎么工作的？"
WebFetch https://deepwiki.com/facebook/react "概述整体架构"
```

### 示例场景

- 了解 Next.js 的 App Router 架构 → `ask_question("vercel/next.js", "How does App Router work?")`
- 查看 React 的文档结构 → `read_wiki_structure("facebook/react")`
- 获取某个模块的详细文档 → `read_wiki_contents("owner/repo", "topic-id")`

### 注意

- 仅支持公开仓库
- 覆盖面有限——如果返回空结果或内容不足，切换到 gh CLI
...
- 大型仓库（>100MB）建议优先使用 DeepWiki 或 gh CLI
- 克隆产物存放在 `/tmp/external-repos/`，重启后可能丢失
- 完成后可用 `--cleanup` 释放空间

## 限制

- **DeepWiki**：仅公开仓库，覆盖面有限（约 5 万热门仓库）
