...
--output <文件路径或 stdout>
--output-format json | jsonl | tsv | text
```

默认输出格式为 `json`（给 agent 用），`text` 是给人类看的可选模式。不要反过来。
...
### 7.1 文档是 agent 的 API，不是人类的 README

AI-Native CLI 的文档不是一篇给人从头读到尾的教程，而是一个**可分层、可索引、可机器消费**的知识库。Agent 在执行过程中按需检索，只拉取当前任务需要的信息。

### 7.2 文档四层架构

| 层级 | 名称 | 内容 | 典型消费场景 |
|------|------|------|------------|
| L0 | Quick Contract | 命令签名 + 必填参数 + 最小示例 | Agent 首次调用命令前 |
| L1 | Operational Spec | 全部字段定义、类型约束、可选参数、退出码 | Agent 处理边界情况 |
| L2 | Deep Spec | 状态机、去重规则、冲突合并、重试策略、并发模型 | Agent 构建复杂工作流 |
| L3 | Rationale | 设计决策的原因、替代方案、取舍分析 | 开发者扩展或修改工具 |

### 7.3 CLI 内建知识检索

不要逼 agent 把整本文档塞进上下文。提供命令内文档接口：

```bash
# 分层查看帮助
mycli help --topic paper.add --detail level0
mycli help --topic paper.add --detail level1
mycli help --topic paper.add --detail level2

# 查看数据 schema
mycli schema show --name paper_add_input_v1
mycli schema list
...
# 查看命令示例
mycli examples --topic search --output-format json

# 列出所有命令及简述
mycli commands list --output-format json
```

### 7.4 help 输出也是结构化的

```bash
mycli help --topic paper.add --detail level0 --output-format json
```
  ...
  "required_params": [
    {
      ...
      "type": "uri",
      "supported_schemes": ["doi", "arxiv", "s2", "https", "http"],
      "examples": ["doi://10.1234/xxxx", "arxiv://2301.00001"]
    }
  ],
  "example": "mycli paper add --paper-ref doi://10.1234/xxxx",
  "detail_level": "level0",
  "next_levels": ["level1", "level2"]
}
```
...
```json
{
  ...
  "refs": [
    {"topic": "paper.search", "detail": "level1", "section": "pagination"}
  ]
}
```
...
### 8.2 分页用 cursor，不用 offset

Offset 在数据变动时不可靠。使用 opaque cursor：

```bash
mycli paper list --max-items 10 --output-format json
# 响应中包含 "cursor": "abc123"

mycli paper list --cursor abc123 --max-items 10 --output-format json
```
...
密钥绝不通过命令行 flags 传递（会泄露到进程列表和 shell 历史）。推荐方式：
...
□ 枚举值严格校验，未知参数报错
□ 幂等性：相同输入不产生重复副作用
□ help 命令可按层级查询（level0/level1/level2）
□ schema 可通过命令查看
