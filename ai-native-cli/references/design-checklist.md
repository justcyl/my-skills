...
| # | 检查项 | 通过 |
|---|--------|------|
| 1 | 默认输出格式为 JSON（`--output-format json`） | □ |
...
| 9 | 分页使用 opaque cursor，非 offset/page | □ |
| 10 | 文档分层实现（L0 Quick Contract 至少可用） | □ |
| 11 | `--help` 支持 `--output-format json` 输出结构化帮助 | □ |
