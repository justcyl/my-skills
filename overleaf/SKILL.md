...
通过 Git 协议操作 Overleaf 项目文件（克隆、编辑、提交、推送），通过 Review API 管理评论线程（列出、定位、解决）。

所有文件级操作（浏览目录、读写文件、创建删除、下载项目）统一通过 `git clone` + 本地编辑 + `git push` 完成，不再使用 WebSocket/REST 逐文件操作。
...
Git 凭据已配置在 osxkeychain 中，可直接使用 `git clone https://...`。

## 调用方式

Review 操作通过 wrapper 脚本：
...
- `review list/locate/resolve` 依赖 Overleaf 内部评论线程与 `joinDoc` 接口（非官方公开 API），不同私有部署可能有差异
