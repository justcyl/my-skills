...
```bash
ARXIV_ID=<arxiv_id>   # 例： 2508.05004

# 搜索飞书文档（结果在 results 字段，不是 items）
...
`--api-version v2 --doc-format markdown` 创建的文档，内部 `<title>` 元素默认为 "Untitled"。该元素的 id 是**文档 id 本身**（不是 `doxcn...` 块 id），无法通过 `block_replace` 修改。**必须用 `drive files patch`：**
...
| 场景 | 正确做法 |
|------|----------|
| `docs +create --api-version v2` 的 `--title` | 只设 Drive 文件名，不更新内部 page title（默认 Untitled）；内部 `<title>` 元素的 id = 文档 id 本身，无法用 block_replace 修改 | 创建后执行 **Step 3-title**（`drive files patch`）修复 |
