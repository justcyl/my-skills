...
```bash
# 在终端里以 8x 速度回放
python alan-pipeline/scripts/replay.py runs/run-1_20260419.jsonl --speed 8

# 慢放关键阶段
python alan-pipeline/scripts/replay.py runs/run-1_20260419.jsonl --speed 0.5

# 即时（无延迟，快速浏览最终状态）
python alan-pipeline/scripts/replay.py runs/run-1_20260419.jsonl --speed 0
```
