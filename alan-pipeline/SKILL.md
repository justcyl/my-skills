...
```yaml
# .pipeline/runs/<slug>.yaml — 三个核心字段
...
```bash
write .pipeline/runs/verify-debiased-k1.yaml
```
...
可选字段：`model`（指定模型）、`thinking`（思考等级）、`tags`（标签）。
...
```bash
read .pipeline/runs/<slug>.state       # 轮次、状态
read .pipeline/runs/<slug>.progress.md  # 每轮记录
```
...
```json
{"action": "run", "pane": "run-monitor", "command": "watch -n 30 'cat .pipeline/runs/<slug>.state && echo --- && tail -5 .pipeline/runs/<slug>.progress.md'"}
```
