---
name: siyuan-cli
description: 操作思源笔记的 CLI 工具。当用户要求管理思源笔记的笔记本、文档、块、标签、搜索、SQL 查询、导入导出、资源、同步，或配置 auth、AI chat、MCP Server 时使用此 skill。
---

# siyuan-cli 使用指南

可执行文件：全局命令 `siyuan-cli`
全局选项：`--format json` 输出 JSON；支持 `--output/-o <file>` 的命令可导出到文件

## 前置条件

思源笔记实例需已运行，默认地址为 `http://127.0.0.1:6806`。首次使用或连接失败时，先执行：

```bash
siyuan-cli auth login --url http://127.0.0.1:6806 --token <api-token>
siyuan-cli auth status
```

API token 在思源笔记「设置 -> 关于 -> API token」获取。不要在回复中展示用户的 token。
...
siyuan-cli auth status                                # 查看连接状态
siyuan-cli auth logout                                # 清除连接配置
siyuan-cli config list                                 # 查看配置
siyuan-cli config get <key>                            # 获取配置项
siyuan-cli config set <key> <value>                    # 设置配置项
siyuan-cli chat "问题"                                 # 基于笔记内容单轮问答
siyuan-cli chat                                        # 进入多轮对话
siyuan-cli mcp                                         # 启动 stdio MCP Server
siyuan-cli version                                     # 版本信息
```

AI 对话需要额外配置兼容 OpenAI 的模型服务：

```bash
siyuan-cli config set ai.base_url <base-url>
siyuan-cli config set ai.api_key <api-key>
siyuan-cli config set ai.model <model>
```
...
- 涉及删除、回滚、清理资源、同步等会改变数据的命令，执行前向用户确认目标和范围
- 不要泄露 `api_token`、`ai.api_key` 或配置文件中的其他凭据
