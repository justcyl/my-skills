...
## Specialized Skills (Local References)

For complex scenarios, **read the dedicated skill file** from the `skills/` directory bundled with this skill.
All paths below are relative to this SKILL.md's parent directory.

**Task-matching rule:** When the user's request matches a specialized skill's scope, read that skill's `SKILL.md` (and its companion `.md` files) before proceeding. The specialized skills contain detailed workflows, property references, and recipes not covered in this general reference.

### Format-Specific Skills (read when working with a specific format)

| Trigger | Read | Companion Files | Scope |
|---------|------|-----------------|-------|
| Any `.docx` task | `skills/officecli-docx/SKILL.md` | `creating.md`, `editing.md` | Word documents — reports, letters, memos, proposals |
| Any `.pptx` task | `skills/officecli-pptx/SKILL.md` | `creating.md`, `editing.md` | Presentations — general slide decks |
| Any `.xlsx` task | `skills/officecli-xlsx/SKILL.md` | `creating.md`, `editing.md` | Excel — formulas, charts, trackers |

### Domain-Specific Skills (read when matching the domain)

| Trigger | Read | Companion Files | Scope |
|---------|------|-----------------|-------|
| Academic paper / research paper / white paper / TOC + equations + footnotes | `skills/officecli-academic-paper/SKILL.md` | `creating.md` | Academic papers with TOC, equations, footnotes, bibliography |
| Pitch deck / investor deck / sales deck / product launch slides | `skills/officecli-pitch-deck/SKILL.md` | `creating.md` | Investor/product/sales decks with charts and callouts |
| Data dashboard / KPI report / CSV to Excel visualization | `skills/officecli-data-dashboard/SKILL.md` | `creating.md` | CSV/tabular data → Excel dashboards with charts, sparklines |
| Financial model / DCF / 3-statement model / cap table | `skills/officecli-financial-model/SKILL.md` | `creating.md` | Formula-driven multi-sheet financial models |
| Morph transition / cinematic animation / smooth PPT | `skills/morph-ppt/SKILL.md` | `reference/` directory (design rules, quality gates, 40+ style templates) | Morph-animated cinematic presentations |

---

## Examples (Local References)

The `examples/` directory bundled with this skill contains runnable scripts and documentation.
**When you need a recipe or pattern**, read the relevant example before writing commands from scratch.

### Word Examples (`examples/word/`)

| File | What it demonstrates |
|------|---------------------|
| `formulas.sh` / `formulas.md` | LaTeX math formulas in Word |
| `tables.sh` / `tables.md` | Complex styled tables |
| `textbox.sh` / `textbox.md` | Formatted text boxes |

### Excel Examples (`examples/excel/`)

| File | What it demonstrates |
|------|---------------------|
| `charts.sh` / `charts.md` | Professional charts overview |
| `charts-demo.sh` / `charts-demo.md` | 14+ chart types demo |
| `charts-*.py` / `charts-*.md` | Individual chart types (bar, line, pie, scatter, bubble, radar, waterfall, histogram, stock, combo, area, box-whisker, column, advanced, basic, extended) |
| `pivot-tables.py` / `pivot-tables.md` | Pivot table creation |

### PowerPoint Examples (`examples/ppt/`)

| File | What it demonstrates |
|------|---------------------|
| `presentation.sh` / `presentation.md` | Basic presentation creation |
| `animations.sh` / `animations.md` | Animation effects |
| `video.py` / `video.md` | Video embedding |
| `3d-model.sh` / `3d-model.md` | 3D model embedding |
| `templates/` | 14+ professional style templates with build scripts |
