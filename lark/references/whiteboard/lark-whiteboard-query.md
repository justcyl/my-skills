...
- `raw`：飞书 OpenAPI 原生画板节点格式。这一 json 格式不适合直接编辑复杂布局或内容，建议仅限于需要修改简单的文本内容/颜色等细节时使用。需要进行更复杂的设计/修改时，优先考虑 Mermaid/PlantUML；若确需外部 whiteboard-cli helper，必须先取得用户明确同意。
