...
description: Codex 专用：将学术论文整理为以原文逐段中文翻译为主体、再叠加精读注释的飞书文档。触发语境：arXiv/DOI/PDF 论文到飞书、帮我读这篇论文、给 paper 做 Codex 版飞书笔记。
metadata:
  requires:
    bins: ["lark-cli"]
...
把一篇论文整理成可直接阅读的飞书论文翻译精读文档。唯一交付形态是：以原论文正文的中文逐段忠实翻译为主体，按原论文结构保留正文顺序，原位保留图、表、公式、算法和附录；然后在这个翻译主体之上叠加导读、作者思考路径、图表读法、公式直觉、术语边注、引用背景、代码映射等阅读层。“加法”只表示在原文翻译上追加注释层，不表示把论文改写成脱离原文顺序的精读讲义、总结文档或解读文档。这个版本面向 Codex 执行，不使用 Pi、Herdr pane 或固定模型子代理；默认直接使用 Codex 当前可用的子代理/多代理/并行工作者能力，不逐次询问。若当前环境确无可调用子代理工具，才降级为 Codex 本体分批处理，并且降级记录只写入内部 checkpoint/QC，不写入正式文档。
...
多智能体/并行工作者用于加速和交叉审查论文阅读层，但主 agent 必须统一复核、合并和落盘，不能把候选内容未经检查直接写入正式文档。

- 翻译与覆盖：按章节或段落分片生成忠实中文翻译候选，检查是否遗漏摘要、方法、实验、相关工作、结论和附录。
- 术语与边注：提取核心术语、缩写、数学概念和高语义载荷段落，生成 comment 候选与唯一定位文本。
- 阅读层候选：为导读、作者思考路径、图表读法、公式直觉、方法具象化、引用背景、实现映射和读者疑问生成候选。
...
本 skill 的正式文档首先是一份论文原文翻译；精读内容只是叠加在翻译上的阅读辅助。执行时必须先完成可独立阅读的逐段翻译主体，再添加 callout/comment。
...
- 阅读层必须锚定到已有翻译段落、公式、图、表、算法或引用之后；它是“在翻译上补充”，不是另起一份加法文档。
...
- 加法阅读层不能改写或替代正文。导读、作者思考路径、图表读法、公式直觉、引用背景、实现映射和读者疑问必须进入 callout/comment 等阅读层，不混入原文翻译段落。
- 术语可以统一译名，但不要把术语表内容扩写进正文；正文只负责翻译原文，不负责讲解原文。

## Additive Reading Contract

注释版的目标不是压缩论文，也不是另写一篇精读讲义，而是在完整原文翻译的基础上做加法。所有阅读层必须锚定到具体翻译段落、公式、图、表、算法或引用，帮助读者回到原文继续读；不得用摘要替代原文、删减关键限定、把辅助推断写成作者结论。
...
用户给出 arXiv ID、DOI、论文 PDF 或论文 URL 时，只产出一种文档：飞书论文翻译精读文档。不存在“只翻译不注释”的分支；但注释必须叠加在完整翻译之上，不能把正文改造成纯解读稿。如果用户明确要求不要上传飞书或只要本地短答，再退出本 skill，改用普通回答或 `ph-paper-helper`。
...
4. **Plan The Document**
   ...
   - 在 `translation-plan.md` 首行写明 `Deliverable: 飞书论文翻译精读文档（原文翻译为主体，注释层为加法）`，并列出正文翻译覆盖项与阅读层覆盖项。
...
5. **Translate**
   ...
   - 不要主动改写成总结、导读、解读、评论或精读讲义；加法内容后续只能进入 callout/comment。
...
10. **Quality Check (QC) And Visual Gate**
    ...
    - 翻译主体检查：正文是否仍是按原论文结构逐段翻译，是否有用总结、导读、解读或精读讲义替代原文翻译的段落；发现后必须恢复为翻译正文。
    - 加法精读检查：是否包含导读、作者思考路径、图表读法、公式直觉、引用背景、实验读法、局限、代码映射（若有仓库）和附录覆盖；同时检查这些阅读层没有混入正文翻译段落。
