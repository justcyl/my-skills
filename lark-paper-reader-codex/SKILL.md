...
把一篇论文整理成可直接阅读的飞书加法精读翻译文档。唯一交付形态是：按原论文结构保留正文顺序，提供中文忠实翻译或忠实转述，原位保留图、表、公式、算法和附录，并在不替代原文的前提下加入导读、作者思考路径、图表读法、公式直觉、术语边注、引用背景、代码映射等阅读层。这个版本面向 Codex 执行，不使用 Pi、Herdr pane 或固定模型子代理；默认直接使用 Codex 当前可用的子代理/多代理/并行工作者能力，不逐次询问。若当前环境确无可调用子代理工具，才降级为 Codex 本体分批处理，并且降级记录只写入内部 checkpoint/QC，不写入正式文档。
